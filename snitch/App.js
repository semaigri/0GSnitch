...
let privateKey = key
 ...
 function bytesToImage(bytes, mime = 'image/png') {
  const base64 = btoa(String.fromCharCode(...bytes));
  return `data:${mime};base64,${base64}`;
}
export async function updateMessages(setMessage){
  const value = await AsyncStorage.getItem(`msgs${chosenUser}`);
  const x = JSON.parse(value) || [];
  console.log("asked for chat history", x);

  let array = [];
  for (let i = 0; i < x.length; i++) {
    console.log("(x[i].msgType,i:", i, "mT:", (x[i].msgType))

    if (x[i].msgType == "received") {
      array.push(
        <Text key={i} style={{
          fontSize: 17,
          backgroundColor: 'grey', // soft blue
          color: "white",
          marginTop: 6,
          paddingVertical: 16,
    ...
    else if (x[i].msgType === "image") {
      // keep your image base64 conversion here
      array.push(/* your image JSX */);
    }
    else if (x[i].msgType === "video") {
      let src;
      if (Platform.OS === 'web') {
        const res = await fetch(x[i].text);
      ...
      array.push(
        Platform.OS === 'web' ? (
          <video key={`vid-${i}`} src={src} style={{ width:'100%', height:300 }} controls />
        ) : (
          <Video key={`vid-${i}`} source={{ uri: src }} style={{ width:'100%', height:300 }} resizeMode="contain" controls />
        )
      );
    }
        
      else if(x[i].msgType=="sent"){
        console.log("sent text detected")
        array.push(
          <Text key={i} style={{
            fontSize: 17,
    ...
    setMessage(array);
    console.log("1PUSHING NOW", array)
  ...
  const ViewMyChats = function(){
    ...
    for(let i=0; i<myChatsArr.length; i++){
      vsArr.push(
          ...
          onPress={() => {
            chosenUser = i+1
            chosenUserName = myChatsArr[i].itsName
            chosenUserNickTag = myChatsArr[i].itsUserName
            alert(i+1)
            setScreen('SendMessage')
            setUserId(i+1)
            updateMessages(setMessage)
          ...
          <ScrollView contentContainerStyle={{ paddingBottom: 90 }}>             
            <View style={[styles.userListFlex, styles.textStyle]}>
              <View style={[styles.textStyle,{ flex:1 }]}>
                { Array.isArray(vsArr) ? vsArr.slice(0, -1) : vsArr }              
              </View>
            </View>
          </ScrollView>
      
          {/* render the last pushed element (your bottom bar) OUTSIDE the ScrollView so it's pinned */}
          { Array.isArray(vsArr) ? vsArr[vsArr.vsArr - 1] : null }               
        </View>
    )
  }
  const SendFileCall = function(receiver){
...
//setMessage(array);
    elements.push([
      // Заголовок (сверху), аккуратный «хакерский» стиль
      <Text style={{
        fontSize: 28,
        textAlign: 'center',
        color: 'rgb(96,241,11)',
...
let uri, bytesMedia
//view by bytes
async function viewFileBytesWeb(fileOrUrl, maxBytes = 64) {
  // fileOrUrl = File | Blob | string(URL)
  let ab;
  ...
  const preview = Array.from(bytes.slice(0, maxBytes))
    ...
    .join(' ');
  console.log(`bytes length: ${bytes.length}, preview (hex):`, preview);
  console.log("BYTES of media:", bytes)
  bytesMedia=bytes
  return bytes;
}
let chosenDataType
  ...
  return new Promise(resolve => {
    if (Platform.OS === 'web') {
      ...
      input.onchange = () => {
        ...
        chosenDataType=f.type.slice(0,5)
         uri = f ? URL.createObjectURL(f) : null;
         viewFileBytesWeb(uri, 64)
        input.remove();
        ...
        setMedia(
          ...
          { /* preview */}
            <View style={{ width: '92%', maxHeight: '70%', marginBottom: 20 }}>
              { Platform.OS === 'web' ? (
                <img
                  src={uri}
                  style={{ width: '100%', height: 300, borderRadius: 8, objectFit: 'contain' }}
                  alt="preview"
                />
              ) : (
                <Image
            ...
            <View style={{ width: '92%', flexDirection: 'row', justifyContent: 'space-between' }}>
              <Pressable
                onPress={() => { setMedia(null); resolve(null); }}
                style={{ flex: 1, marginRight: 8, padding: 14, borderRadius: 10, backgroundColor: '#333', alignItems: 'center' }}
              >
                <Text style={{ color: '#fff', fontSize: 16 }}>Cancel</Text>
              </Pressable>
        
              <Pressable
                onPress={() => { setMedia(null); resolve(uri);  if(SendMessage(bytesMedia, nodeIp, serverIp, chosenUser, ReceiverPublicKey, chosenDataType, uri)){
                  setTimeout(()=>{updateMessages(setMessage)}, 180)
                }
               }}
                style={{ flex: 1, marginLeft: 8, padding: 14, borderRadius: 10, backgroundColor: '#00ff88', alignItems: 'center' }}
              >
                <Text style={{ color: '#000', fontSize: 16, fontWeight: '700' }}>Send</Text>
              </Pressable>
            </View>
          </View>
        );
...
<Pressable onPress={() => pickFile().then(uri => { fileUrl = uri; console.log('picked uri:', uri); })} style={{ color: 'white', fontSize: 24, marginRight: 10 }}>
<Text style={{ color: 'white', fontSize: 24 }}>+</Text>
  ...
  {/* опционально: кнопка Send справа */}
       ...
       <Pressable 
      onPress={()=>{
        if(SendMessage(inputValue, nodeIp, serverIp, chosenUser, ReceiverPublicKey, "sent", null)){
          setTimeout(()=>{updateMessages(setMessage)}, 180)
        }
